\section{Act VI --- Robustness: The Benchmark Survives Its Own Tests}\label{act-vi-robustness-the-benchmark-survives-its-own-tests}
...
\subsection{Protocol Independence: Model Identity Drives Structure}\label{protocol-independence-model-identity-drives-structure}
...
\textbf{Statistical caveat.} The p-values use a chi-squared approximation to the F-distribution (ignoring denominator degrees of freedom) and treat per-pair cells as independent without repeated-measures modeling. Only 3 models were tested (Claude, GPT, DeepSeek). The effect sizes (η²) are more reliable than the exact p-values. The qualitative conclusion --- that model identity dominates while protocol parameters are null --- is robust to the approximation, but the analysis should be understood as descriptive/exploratory rather than confirmatory.
...
\subsection{Bridge Frequency Is the Most Robust Property}\label{bridge-frequency-is-the-most-robust-property}
...
\subsection{Gait Rankings Are Largely Stable}\label{gait-rankings-are-largely-stable}
...
\bottomrule\noalign{}
\endlastfoot
7wp-t0.7 (baseline) & Claude \textgreater{} GPT \textgreater{} DeepSeek \\
5wp-t0.5 & Claude \textgreater{} DeepSeek \textgreater{} GPT \\
5wp-t0.9 & Claude \textgreater{} DeepSeek \textgreater{} GPT \\
9wp-t0.5 & Claude \textgreater{} DeepSeek \textgreater{} GPT \\
9wp-t0.9 & Claude \textgreater{} DeepSeek \textgreater{} GPT \\
\end{longtable}
...
\subsection{Asymmetry Is Resolution-Dependent}\label{asymmetry-is-resolution-dependent}

Mean asymmetry per condition:

\begin{longtable}[]{@{}lrc@{}}
\toprule\noalign{}
Condition & Mean Asymmetry & \textgreater{} 0.60 Threshold? \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
5wp-t0.5 & 0.594 & no \\
5wp-t0.9 & 0.593 & no \\
9wp-t0.5 & 0.669 & \textbf{yes} \\
9wp-t0.9 & 0.684 & \textbf{yes} \\
\end{longtable}
