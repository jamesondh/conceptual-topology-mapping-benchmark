#!/usr/bin/env bun
/**
 * Phase 8A: Bridge Fragility Experiment
 *
 * Tests whether bridge survival under pre-fill perturbation correlates
 * negatively with competitor count. Two-stage design:
 *
 * Stage 1 (Salience): Collect unconstrained waypoint paths for 8 new pairs
 *   to measure salience landscapes and competitor counts.
 *   20 reps x 4 models x 8 pairs = 640 runs (via Scheduler)
 *
 * Stage 2 (Pre-fill): Collect pre-filled paths for the same 8 pairs to
 *   measure bridge survival under incongruent perturbation.
 *   10 reps x 4 models x 8 pairs = 320 runs (custom elicitation)
 *
 * Total: ~960 new runs + retry buffer ~ 1,010
 *
 * Usage:
 *   bun run experiments/08a-fragility.ts
 *   bun run experiments/08a-fragility.ts --dry-run
 *   bun run experiments/08a-fragility.ts --models claude,gemini
 */
...
import { PHASE8A_PAIRS } from "../src/data/pairs-phase8.ts";
...
function makeSyntheticPair(
  ...
  return {
    ...
    set: "reporting",
    notes: `Phase 8A fragility: "${pair.from}" -> "${pair.to}" (stage: ${suffix}, predicted bridge: ${pair.predictedBridge}, competitors: ${pair.predictedCompetitorCount})`,
  };
}
...
async function loadExistingResults(
  ...
  for (const file of files) {
    if (!file.endsWith(".json") || file.startsWith("fragility-")) continue;
...
async function main() {
  ...
  program
    .name("08a-fragility")
    .description(
      "Run Phase 8A bridge fragility experiment: salience landscapes + pre-fill survival",
    )
  ...
  const fragilityDir = path.join(outputDir, "fragility");
  ...
  // Print header
  console.log("=== Phase 8A: Bridge Fragility Experiment ===\n");
  console.log(`Total pairs:             ${PHASE8A_PAIRS.length}`);
  console.log(`Stage 1 (salience):      ${PHASE8A_PAIRS[0].targetReps.salience} reps/model (unconstrained, ${WAYPOINT_COUNT} waypoints)`);
  console.log(`Stage 2 (pre-fill):      ${PHASE8A_PAIRS[0].targetReps.preFill} reps/model (pre-filled, ${WAYPOINT_COUNT} total waypoints)`);
  ...
  for (const pair of PHASE8A_PAIRS) {
    console.log(
      `  ${pair.id} (${pair.competitorLevel}): "${pair.from}" -> "${pair.to}" | ` +
      `predicted bridge: "${pair.predictedBridge}", competitors: ${pair.predictedCompetitorCount} | ` +
  ...
  const existing = await loadExistingResults(fragilityDir);
  if (existing.results.length > 0) {
    console.log(`Found ${existing.results.length} existing results in ${fragilityDir}/`);
    console.log("");
  }

  // -- Build the run manifest --

  // Stage 1: Salience runs (unconstrained, via Scheduler)
  ...
  // Budget breakdown
  const targetSalience = PHASE8A_PAIRS.length * PHASE8A_PAIRS[0].targetReps.salience * models.length;
  const targetPreFill = PHASE8A_PAIRS.length * PHASE8A_PAIRS[0].targetReps.preFill * models.length;

  console.log("Run budget:");
  console.log(`  Stage 1 (salience):     ${PHASE8A_PAIRS.length} pairs x ${PHASE8A_PAIRS[0].targetReps.salience} reps x ${models.length} models = ${targetSalience}`);
  console.log(`  Stage 2 (pre-fill):     ${PHASE8A_PAIRS.length} pairs x ${PHASE8A_PAIRS[0].targetReps.preFill} reps x ${models.length} models = ${targetPreFill}`);
  console.log(`  Target total:           ${targetSalience + targetPreFill}`);
  console.log("");
  ...
  await mkdir(fragilityDir, { recursive: true });
  ...
  async function writeResult(result: ElicitationResult): Promise<void> {
    const resultPath = path.join(fragilityDir, `${result.runId}.json`);
    try {
  ...
  // -- Stage 1: Run salience requests via Scheduler --

  if (salienceRequests.length > 0) {
    console.log(`Running ${salienceRequests.length} salience requests (Stage 1)...`);
    ...
    console.log(`\n  Stage 1 (salience) complete: ${salienceResults.length} results\n`);
  }
  ...
  console.log("=== Phase 8A: Bridge Fragility Experiment Complete ===\n");
  ...
  // Write summary
  const summaryPath = path.join(fragilityDir, "fragility-summary.json");
  const summary = {
    experiment: "fragility",
    phase: "8A",
    startedAt: new Date(experimentStartTime).toISOString(),
    completedAt: new Date().toISOString(),
    durationMs: duration,
    pairs: PHASE8A_PAIRS.map((p) => p.id),
  ...
  console.log(`Results: ${fragilityDir}/`);
