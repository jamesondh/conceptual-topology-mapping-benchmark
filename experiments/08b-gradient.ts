#!/usr/bin/env bun
/**
 * Phase 8B: Gradient-Midpoint & Causal-Chain Waypoint Collection
 *
 * Single-stage experiment collecting unconstrained waypoint paths (5 waypoints)
 * for 20 pairs: 10 gradient-midpoint + 10 causal-chain.
 *
 * Total runs: 20 pairs x 15 reps x 4 models = 1,200 runs + retry buffer ~1,280
 *
 * Usage:
 *   bun run experiments/08b-gradient.ts
 *   bun run experiments/08b-gradient.ts --dry-run
 *   bun run experiments/08b-gradient.ts --models claude,gemini
 */
...
function makeSyntheticPair(pair: Phase8GradientPair): ConceptPair {
  return {
    id: pair.id,
    from: pair.from,
    to: pair.to,
    ...
    set: "reporting",
    notes: `Phase 8B ${pair.pairType}: ${pair.from} -> ${pair.to} (candidate bridge: ${pair.candidateBridge})`,
  };
}
...
async function loadExistingResults(
  ...
  for (const file of files) {
    if (!file.endsWith(".json") || file.startsWith("gradient-")) continue;
...
async function main() {
  ...
  program
    .name("08b-gradient")
    .description(
      "Run Phase 8B gradient-midpoint & causal-chain waypoint collection: unconstrained paths for 20 pairs",
    )
  ...
  // Print header
  const gradientCount = PHASE8B_GRADIENT_PAIRS.length;
  const causalCount = PHASE8B_CAUSAL_PAIRS.length;

  console.log("=== Phase 8B: Gradient-Midpoint & Causal-Chain Waypoint Collection ===\n");
  console.log(`Pairs:                ${PHASE8B_ALL_PAIRS.length} (${gradientCount} gradient-midpoint, ${causalCount} causal-chain)`);
  ...
  for (const pair of PHASE8B_GRADIENT_PAIRS) {
    const repl = pair.isPhase7CReplication ? " [7C replication]" : "";
    console.log(`    ${pair.id}: "${pair.from}" -> "${pair.to}" (bridge: "${pair.candidateBridge}", dim: ${pair.dimension})${repl}`);
  }
  console.log(`  Causal-chain pairs (${causalCount}):`);
  for (const pair of PHASE8B_CAUSAL_PAIRS) {
    console.log(`    ${pair.id}: "${pair.from}" -> "${pair.to}" (bridge: "${pair.candidateBridge}", process: ${pair.process})`);
  }
  console.log("");

  // Load existing results for resume support
  const existing = await loadExistingResults(gradientDir);
  if (existing.results.length > 0) {
    console.log(`Found ${existing.results.length} existing results in ${gradientDir}/`);
    console.log("");
  }
  ...
  await mkdir(gradientDir, { recursive: true });
  ...
  const scheduler = new Scheduler(
    { globalConcurrency, perModelConcurrency, throttleMs },
    {
      onResult: async (result: ElicitationResult) => {
        ...
        // Atomic write
        const resultPath = path.join(gradientDir, `${result.runId}.json`);
        try {
  ...
  console.log("=== Phase 8B: Gradient-Midpoint & Causal-Chain Collection Complete ===\n");
  ...
  // Write summary
  const summaryPath = path.join(gradientDir, "gradient-summary.json");
  const summary = {
    experiment: "gradient",
    phase: "8B",
    ...
    gradientPairs: PHASE8B_GRADIENT_PAIRS.map((p) => p.id),
    causalPairs: PHASE8B_CAUSAL_PAIRS.map((p) => p.id),
    gradientPairCount: gradientCount,
    causalPairCount: causalCount,
    models: models.map((m) => m.id),
    waypointCount: WAYPOINT_COUNT,
    repsPerPair: Object.fromEntries(PHASE8B_ALL_PAIRS.map((p) => [p.id, p.targetReps])),
  ...
  console.log(`Results: ${gradientDir}/`);
