#!/usr/bin/env bun
/**
 * Phase 9B: Transformation-Chain Blindness Waypoint Collection
 *
 * Single-stage experiment collecting unconstrained waypoint paths (5 waypoints)
 * for 20 pairs: 10 transformation-chain + 10 gradient-midpoint.
 *
 * Tests whether Gemini shows a specific deficit on transformation-chain bridges
 * while succeeding on gradient-midpoint bridges.
 *
 * Total runs: 20 pairs x 15 reps x 4 models = 1,200 runs + retry buffer ~1,280
 *
 * Usage:
 *   bun run experiments/09b-transformation.ts
 *   bun run experiments/09b-transformation.ts --dry-run
 *   bun run experiments/09b-transformation.ts --models claude,gemini
 */
...
function makeSyntheticPair(pair: Phase9TransformationPair): ConceptPair {
  return {
    id: pair.id,
    from: pair.from,
    to: pair.to,
    ...
    set: "reporting",
    notes: `Phase 9B ${pair.pairType}: ${pair.from} -> ${pair.to} (candidate bridge: ${pair.candidateBridge})`,
  };
}
...
async function loadExistingResults(
  ...
  for (const file of files) {
    if (!file.endsWith(".json") || file.startsWith("transformation-")) continue;
...
async function main() {
  ...
  program
    .name("09b-transformation")
    .description(
      "Run Phase 9B transformation-chain & gradient-midpoint waypoint collection: unconstrained paths for 20 pairs",
    )
  ...
  const transformationDir = path.join(outputDir, "transformation");
  ...
  // Print header
  const transformationCount = PHASE9B_TRANSFORMATION_PAIRS.length;
  const gradientCount = PHASE9B_GRADIENT_PAIRS.length;

  console.log("=== Phase 9B: Transformation-Chain Blindness Waypoint Collection ===\n");
  console.log(`Pairs:                ${PHASE9B_ALL_PAIRS.length} (${transformationCount} transformation-chain, ${gradientCount} gradient-midpoint)`);
  console.log(`Waypoints per path:   ${WAYPOINT_COUNT}`);
  console.log(`Reps per pair/model:  15`);
  console.log(`Models:               ${models.map((m) => m.displayName).join(", ")}`);
  console.log("");

  // Summarize pairs
  console.log("Pair plan:");
  console.log(`  Transformation-chain pairs (${transformationCount}):`);
  for (const pair of PHASE9B_TRANSFORMATION_PAIRS) {
    console.log(`    ${pair.id}: "${pair.from}" -> "${pair.to}" (bridge: "${pair.candidateBridge}", process: ${pair.processOrDimension})`);
  }
  console.log(`  Gradient-midpoint pairs (${gradientCount}):`);
  for (const pair of PHASE9B_GRADIENT_PAIRS) {
    console.log(`    ${pair.id}: "${pair.from}" -> "${pair.to}" (bridge: "${pair.candidateBridge}", dim: ${pair.processOrDimension})`);
  }
  console.log("");

  // Load existing results for resume support
  const existing = await loadExistingResults(transformationDir);
  if (existing.results.length > 0) {
    console.log(`Found ${existing.results.length} existing results in ${transformationDir}/`);
    console.log("");
  }
  ...
  await mkdir(transformationDir, { recursive: true });
  ...
  const scheduler = new Scheduler(
    { globalConcurrency, perModelConcurrency, throttleMs },
    {
      onResult: async (result: ElicitationResult) => {
        ...
        // Atomic write
        const resultPath = path.join(transformationDir, `${result.runId}.json`);
        try {
  ...
  console.log("=== Phase 9B: Transformation-Chain Blindness Collection Complete ===\n");
  ...
  // Write summary
  const summaryPath = path.join(transformationDir, "transformation-summary.json");
  const summary = {
    ...
    durationMs: duration,
    pairs: PHASE9B_ALL_PAIRS.map((p) => p.id),
    transformationPairs: PHASE9B_TRANSFORMATION_PAIRS.map((p) => p.id),
    gradientPairs: PHASE9B_GRADIENT_PAIRS.map((p) => p.id),
    transformationPairCount: transformationCount,
    gradientPairCount: gradientCount,
    models: models.map((m) => m.id),
    waypointCount: WAYPOINT_COUNT,
    repsPerPair: Object.fromEntries(PHASE9B_ALL_PAIRS.map((p) => [p.id, p.targetReps])),
  ...
  console.log(`Results: ${transformationDir}/`);
