#!/usr/bin/env bun
/**
 * Phase 5A: Cue-Strength Gradient Experiment
 *
 * For each of 14 Phase 5A concept triples (A, B, C), collects waypoint paths
 * for forward legs: A->B, B->C, A->C. Each triple belongs to a family
 * (physical-causation, biological-growth, compositional-hierarchy, abstract-affect)
 * and has a cue-strength level (very-high, high, medium, low).
 *
 * Target reps vary by leg: AC=20, AB=10, BC=10 for diagnostic triples;
 * AC=10, AB=10, BC=10 for controls (~1,680 total runs).
 *
 * Top-up logic:
 * - Legs marked via reusableLegsWithSource are topped up: existing runs
 *   across all result dirs count toward targetReps
 * - Fully new legs get targetReps new runs per model
 *
 * Usage:
 *   bun run experiments/05a-cue-strength.ts
 *   bun run experiments/05a-cue-strength.ts --dry-run
 *   bun run experiments/05a-cue-strength.ts --models claude,gemini
 */
...
const REUSE_SUBDIRS = ["pilot", "reversals", "transitivity", "targeted-bridges", "cue-strength"];
...
function makeLegPair(
  ...
  return {
    ...
    set: "reporting",
    notes: `Phase 5A cue-strength leg: ${from} -> ${to} (triple: ${tripleId})`,
  };
}
...
/**
 * Load existing results from the cue-strength output directory.
...
async function loadExistingResults(
  ...
  for (const file of files) {
    if (!file.endsWith(".json") || file.startsWith("cue-strength-")) continue;
 ...
 * targeted-bridges, cue-strength) and count successful runs per (pairId, modelId).
 * Used for top-up accounting.
 */
...
async function main() {
  ...
  program
    .name("05a-cue-strength")
    .description(
      "Run Phase 5A cue-strength gradient experiment: forward legs for Phase 5A triples",
    )
  ...
  const cueStrengthDir = path.join(outputDir, "cue-strength");
  ...
  // Print header
  console.log("=== Phase 5A: Cue-Strength Gradient Experiment ===\n");
  console.log(`Triples:              ${PHASE5A_TRIPLES.length}`);
  ...
  const existing = await loadExistingResults(cueStrengthDir);
  if (existing.results.length > 0) {
    console.log(`Found ${existing.results.length} existing results in ${cueStrengthDir}/`);
    console.log("");
  }
  ...
  for (const triple of PHASE5A_TRIPLES) {
    ...
    for (const fwd of FORWARD_LEGS) {
      ...
      // Get target reps for this leg
      const targetReps = triple.targetReps[legId];
  ...
  for (const triple of PHASE5A_TRIPLES) {
    ...
    console.log(`  ${triple.id} (family: ${triple.family}, cue: ${triple.cueLevel}, type: ${triple.diagnosticType})`);
    for (const leg of tripleLegs) {
      if (leg.reusePairId) {
        console.log(`    ${leg.legId}: top-up (reuse from "${leg.reusePairId}", target: ${leg.targetReps})`);
        totalTopUp++;
      } else {
        console.log(`    ${leg.legId}: new ("${leg.from}" -> "${leg.to}", target: ${leg.targetReps})`);
        totalNew++;
      }
    }
  }
  console.log("");
  console.log(`Legs: ${totalTopUp} top-up, ${totalNew} new`);
  console.log("");

  // Build requests: for each leg, compute how many new runs are needed
  ...
  for (const leg of legPlans) {
    ...
    for (const model of models) {
      ...
      // 1. Runs from this experiment (cue-strength dir) using the synthetic pair ID
  ...
  await mkdir(cueStrengthDir, { recursive: true });
  ...
  const scheduler = new Scheduler(
    { globalConcurrency, perModelConcurrency, throttleMs },
    {
      onResult: async (result: ElicitationResult) => {
        ...
        // Atomic write
        const resultPath = path.join(cueStrengthDir, `${result.runId}.json`);
        try {
  ...
  console.log("=== Phase 5A: Cue-Strength Gradient Complete ===\n");
  ...
  // Write summary
  const summaryPath = path.join(cueStrengthDir, "cue-strength-summary.json");
  const summary = {
    experiment: "cue-strength",
    startedAt: new Date(startTime).toISOString(),
    completedAt: new Date().toISOString(),
    durationMs: duration,
    triples: PHASE5A_TRIPLES.map((t) => t.id),
    models: models.map((m) => m.id),
    repsPerTriple: Object.fromEntries(PHASE5A_TRIPLES.map((t) => [t.id, t.targetReps])),
  ...
  console.log(`Results: ${cueStrengthDir}/`);
