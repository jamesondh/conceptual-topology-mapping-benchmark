#!/usr/bin/env bun
/**
 * Phase 5B: Dimensionality Probing Experiment
 *
 * For each of 14 Phase 5B dimensionality triples (A, B, C), collects
 * waypoint paths for the AC leg only. The bridge concept B is the focal
 * concept itself -- we measure how often B appears on A->C paths when
 * A and C lie on the same vs. different semantic axes of B.
 *
 * Only AC legs are collected (the primary measurement). AB/BC legs are
 * optional per spec and not collected here.
 *
 * Total runs: 14 triples x 4 models x (10-20 reps) = ~960 runs
 *
 * Usage:
 *   bun run experiments/05b-dimensionality.ts
 *   bun run experiments/05b-dimensionality.ts --dry-run
 *   bun run experiments/05b-dimensionality.ts --models claude,gemini
 */
...
/**
 * Create a synthetic ConceptPair for an AC leg of a dimensionality triple.
 * The pair ID encodes the triple ID and "AC" suffix.
 */
function makeACPair(
  tripleId: string,
  from: string,
  ...
  return {
    ...
    set: "reporting",
    notes: `Phase 5B dimensionality AC leg: ${from} -> ${to} (triple: ${tripleId})`,
  };
}
...
/**
 * Load existing results from the dimensionality output directory.
...
async function loadExistingResults(
  ...
  for (const file of files) {
    if (!file.endsWith(".json") || file.startsWith("dimensionality-")) continue;
...
async function main() {
  ...
  program
    .name("05b-dimensionality")
    .description(
      "Run Phase 5B dimensionality probing experiment: AC legs for dimensionality triples",
    )
  ...
  const dimensionalityDir = path.join(outputDir, "dimensionality");
  ...
  // Print header
  console.log("=== Phase 5B: Dimensionality Probing Experiment ===\n");
  console.log(`Triples:              ${PHASE5B_TRIPLES.length}`);
  console.log(`Models:               ${models.map((m) => m.displayName).join(", ")}`);
  console.log(`Legs per triple:      1 (AC only)`);
  console.log(`Focal concepts:       light, bank, fire + 2 controls`);
  console.log("");

  // Load existing results for resume support
  const existing = await loadExistingResults(dimensionalityDir);
  if (existing.results.length > 0) {
    console.log(`Found ${existing.results.length} existing results in ${dimensionalityDir}/`);
    console.log("");
  }
  ...
  for (const triple of PHASE5B_TRIPLES) {
    legPlans.push({
      ...
      to: triple.C,
      pairId: `${triple.id}--AC`,
      targetReps: triple.targetReps.AC,
  ...
  for (const leg of legPlans) {
    console.log(
      `  ${leg.tripleId}: "${leg.from}" -> "${leg.to}" ` +
        `(focal: ${leg.focalConcept}, pattern: ${leg.axisPattern}, target: ${leg.targetReps} reps)`,
    );
  }
  console.log("");

  // Build requests: for each leg, compute how many new runs are needed per model
  ...
  for (const leg of legPlans) {
    const pair = makeACPair(leg.tripleId, leg.from, leg.to);

    for (const model of models) {
      const key = runKey(leg.pairId, model.id);
      const existingCount = existing.counts.get(key) ?? 0;
      const needed = Math.max(0, leg.targetReps - existingCount);
  ...
  await mkdir(dimensionalityDir, { recursive: true });
  ...
  const scheduler = new Scheduler(
    { globalConcurrency, perModelConcurrency, throttleMs },
    {
      onResult: async (result: ElicitationResult) => {
        ...
        // Atomic write
        const resultPath = path.join(dimensionalityDir, `${result.runId}.json`);
        try {
  ...
  console.log("=== Phase 5B: Dimensionality Probing Complete ===\n");
  ...
  // Write summary
  const summaryPath = path.join(dimensionalityDir, "dimensionality-summary.json");
  const summary = {
    ...
    repsPerTriple: Object.fromEntries(PHASE5B_TRIPLES.map((t) => [t.id, t.targetReps.AC])),
    legsCollected: "AC-only",
  ...
  console.log(`Results: ${dimensionalityDir}/`);
