#!/usr/bin/env bun
/**
 * Phase 10A: Model Generality Experiment
 *
 * Tests whether the benchmark's core structural findings generalize to
 * 5 new models beyond the original 4. Two-stage design:
 *
 * Stage 1 (Probe): For each of 5 new models, run 3 probe requests to
 ...
 *
 * Total budget: ~960 runs across all 5 models.
 *
 * Output directory: results/model-generality/
 *
 * Usage:
 *   bun run experiments/10a-model-generality.ts
 *   bun run experiments/10a-model-generality.ts --dry-run
 *   bun run experiments/10a-model-generality.ts --models minimax,kimi
 */
...
import { NEW_MODELS } from "../src/data/pairs.ts";
import { PHASE10A_ALL_PAIRS, PHASE10A_PROBE_PAIRS } from "../src/data/pairs-phase10.ts";
...
const DEFAULT_MODEL_CONCURRENCY = "minimax=2,kimi=2,glm=2,qwen=2,llama=2";
const DEFAULT_OUTPUT_DIR = "results/model-generality";
...
function makeSyntheticPair(pair: Phase10CorePair): ConceptPair {
  return {
    ...
    set: "reporting",
    notes: `Phase 10A model generality: "${pair.from}" -> "${pair.to}" (${pair.direction})`,
  };
}
...
async function loadExistingResults(
  ...
  for (const file of files) {
    if (!file.endsWith(".json") || file.startsWith("model-generality-")) continue;
...
async function main() {
  ...
  program
    .name("10a-model-generality")
    .description(
      "Run Phase 10A model generality experiment: probe new models + full-run replication",
    )
    ...
    .option(
      ...
      'per-model concurrency, e.g., "minimax=2,kimi=2"',
  ...
  if (opts.models) {
    ...
    models = [];
    for (const id of ids) {
      const model = NEW_MODELS.find((m) => m.id === id);
      if (!model) {
        console.error(`Unknown model "${id}". Valid: ${NEW_MODELS.map((m) => m.id).join(", ")}`);
        process.exit(1);
      }
      models.push(model);
    }
  ...
  console.log(`Max budget:              ${models.length * PHASE10A_ALL_PAIRS.length * FULL_RUN_REPS} runs`);
  console.log("");

  // Summarize pairs
  console.log("Pair plan:");
  for (const pair of PHASE10A_ALL_PAIRS) {
  ...
  if (dryRun) {
    ...
    for (const model of models) {
      console.log(`  Would probe ${model.displayName} with ${PHASE10A_PROBE_PAIRS.length} pairs`);
    }
    console.log("");
    console.log("Full-run stage (depends on probe results):");
    console.log(`  Up to ${models.length} models x ${PHASE10A_ALL_PAIRS.length} pairs x ${FULL_RUN_REPS} reps = ${models.length * PHASE10A_ALL_PAIRS.length * FULL_RUN_REPS} runs`);
    console.log("");
  ...
  if (eligibleModels.length === 0) {
    ...
    const summaryPath = path.join(outputDir, "model-generality-summary.json");
    const summary = {
      experiment: "model-generality",
      phase: "10A",
  ...
  console.log(`  Pairs:                  ${PHASE10A_ALL_PAIRS.length}`);
  ...
  if (totalNewRuns === 0) {
    ...
    console.log("=== Phase 10A: Model Generality Experiment Complete ===\n");
