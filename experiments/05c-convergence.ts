#!/usr/bin/env bun
/**
 * Phase 5C: Triple-Anchor Convergence Experiment
 *
 * For each of 8 convergence pairs, collects 7-waypoint paths in BOTH
 * forward and reverse directions. Tests for W-shaped positional convergence
 * profiles when a bridge concept exists between endpoints.
 *
 ...
 *
 * Usage:
 *   bun run experiments/05c-convergence.ts
 *   bun run experiments/05c-convergence.ts --dry-run
 *   bun run experiments/05c-convergence.ts --models claude,gemini
 */
...
/**
 * Create a synthetic ConceptPair for a convergence direction.
 * The pair ID encodes the convergence pair ID and direction.
 */
function makeDirectionPair(
  convergencePair: Phase5ConvergencePair,
  direction: "fwd" | "rev",
): ConceptPair {
  const from = direction === "fwd" ? convergencePair.from : convergencePair.to;
  const to = direction === "fwd" ? convergencePair.to : convergencePair.from;

  return {
    id: `${convergencePair.id}--${direction}`,
    from,
    to,
    ...
    set: "reporting",
    notes: `Phase 5C convergence: ${from} -> ${to} (pair: ${convergencePair.id}, direction: ${direction})`,
  };
}
...
/**
 * Load existing results from the convergence output directory.
...
async function loadExistingResults(
  ...
  for (const file of files) {
    if (!file.endsWith(".json") || file.startsWith("convergence-")) continue;
...
async function main() {
  ...
  program
    .name("05c-convergence")
    .description(
      "Run Phase 5C triple-anchor convergence experiment: 7-waypoint forward + reverse paths",
    )
  ...
  const convergenceDir = path.join(outputDir, "convergence");
  ...
  // Print header
  console.log("=== Phase 5C: Triple-Anchor Convergence Experiment ===\n");
  console.log(`Pairs:                ${PHASE5C_PAIRS.length}`);
  ...
  for (const pair of PHASE5C_PAIRS) {
    const bridgeInfo = pair.expectedBridge
      ? `bridge: "${pair.expectedBridge}" (${pair.bridgeModels?.join(", ") ?? "all"})`
      : "no expected bridge";
    console.log(`  ${pair.id} (${pair.pairType}): "${pair.from}" <-> "${pair.to}" — ${bridgeInfo}, ${pair.targetReps} reps`);
  }
  console.log("");

  // Load existing results for resume support
  const existing = await loadExistingResults(convergenceDir);
  if (existing.results.length > 0) {
    console.log(`Found ${existing.results.length} existing results in ${convergenceDir}/`);
    console.log("");
  }
  ...
  const scheduler = new Scheduler(
    { globalConcurrency, perModelConcurrency, throttleMs },
    {
      onResult: async (result: ElicitationResult) => {
        ...
        // Atomic write
        const resultPath = path.join(convergenceDir, `${result.runId}.json`);
        try {
  ...
  console.log("=== Phase 5C: Triple-Anchor Convergence Complete ===\n");
  ...
  // Write summary
  const summaryPath = path.join(convergenceDir, "convergence-summary.json");
  const summary = {
    ...
    waypointCount: WAYPOINT_COUNT,
    directions: ["fwd", "rev"],
    repsPerPair: Object.fromEntries(PHASE5C_PAIRS.map((p) => [p.id, p.targetReps])),
  ...
  console.log(`Results: ${convergenceDir}/`);
