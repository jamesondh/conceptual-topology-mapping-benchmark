#!/usr/bin/env bun
/**
 * Phase 6A: Navigational Salience Mapping Experiment
 *
 * For each of 8 focal pairs, collects waypoint paths (5 waypoints) to build
 * empirical waypoint frequency distributions. Bridge-present pairs get 40 reps
 * per model; bridge-absent controls get 30 reps.
 *
 * Total runs: 6 bridge-present pairs x 40 reps x 4 models = 960
 *           + 2 bridge-absent pairs x 30 reps x 4 models = 240
 *           = ~1,200 runs
 *
 * Usage:
 *   bun run experiments/06a-salience.ts
 *   bun run experiments/06a-salience.ts --dry-run
 *   bun run experiments/06a-salience.ts --models claude,gemini
 */
...
function makeSaliencePair(
  ...
  return {
    ...
    set: "reporting",
    notes: `Phase 6A salience mapping: ${from} -> ${to}`,
  };
}
...
async function loadExistingResults(
  ...
  for (const file of files) {
    if (!file.endsWith(".json") || file.startsWith("salience-")) continue;
...
async function main() {
  ...
  program
    .name("06a-salience")
    .description(
      "Run Phase 6A navigational salience mapping experiment: empirical waypoint frequency distributions",
    )
  ...
  const salienceDir = path.join(outputDir, "salience");
  ...
  // Print header
  console.log("=== Phase 6A: Navigational Salience Mapping Experiment ===\n");
  console.log(`Pairs:                ${PHASE6A_PAIRS.length} (${PHASE6A_PAIRS.filter(p => p.category === "bridge-present").length} bridge-present, ${PHASE6A_PAIRS.filter(p => p.category === "bridge-absent").length} bridge-absent)`);
  ...
  for (const pair of PHASE6A_PAIRS) {
    const bridgeInfo = pair.knownBridge
      ? `bridge: "${pair.knownBridge}" (freq: ${pair.bridgeFreqRange})`
      : "no known bridge";
    console.log(`  ${pair.id} (${pair.category}): "${pair.from}" -> "${pair.to}" — ${bridgeInfo}, ${pair.targetReps} reps`);
  }
  console.log("");

  // Load existing results for resume support
  const existing = await loadExistingResults(salienceDir);
  if (existing.results.length > 0) {
    console.log(`Found ${existing.results.length} existing results in ${salienceDir}/`);
    console.log("");
  }
  ...
  for (const pair of PHASE6A_PAIRS) {
    const syntheticPair = makeSaliencePair(pair.id, pair.from, pair.to);
  ...
  await mkdir(salienceDir, { recursive: true });
  ...
  const scheduler = new Scheduler(
    { globalConcurrency, perModelConcurrency, throttleMs },
    {
      onResult: async (result: ElicitationResult) => {
        ...
        // Atomic write
        const resultPath = path.join(salienceDir, `${result.runId}.json`);
        try {
  ...
  console.log("=== Phase 6A: Navigational Salience Mapping Complete ===\n");
  ...
  // Write summary
  const summaryPath = path.join(salienceDir, "salience-summary.json");
  const summary = {
    experiment: "salience",
    phase: "6A",
    ...
    waypointCount: WAYPOINT_COUNT,
    repsPerPair: Object.fromEntries(PHASE6A_PAIRS.map((p) => [p.id, p.targetReps])),
  ...
  console.log(`Results: ${salienceDir}/`);
